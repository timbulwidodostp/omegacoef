...
capture program drop omegacoef
program omegacoef, rclass
version 12.0
...
capture mi svyset
if !_rc {
    di as error "omega does not currently support survey weights."
    exit
    }
else {
    capture svyset
...
qui ssd query
if `r(isSSD)' ==  1 {
    di as error "omega does not currently support summary statistics data."
    exit
    }
...
qui sem (Factor -> `vars'), var(Factor@1) standardized iterate(`iterate') `method' 

if `e(converged)' == 0 {
    di as error "omega could not be computed."
    di "Ensure that all variables are continuous."
    exit = 430
    }
